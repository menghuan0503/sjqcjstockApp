...
public class ImageControlView extends ImageView {
    ...
    public ImageControlView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        // TODO Auto-generated constructor stub
    }

    // ImageView img;
    private Matrix imgMatrix = null; // 定义图片的变换矩阵

    private static final int DOUBLE_CLICK_TIME_SPACE = 300; // 双击时间间隔
    private static final int DOUBLE_POINT_DISTANCE = 10; // 两点放大两点间最小间距
    private static final int NONE = 0;
    private static final int DRAG = 1; // 拖动操作
    private static final int ZOOM = 2; // 放大缩小操作
    private int mode = NONE; // 当前模式

    private float bigScale = 3f; // 默认放大倍数
    private Boolean isBig = false; // 是否是放大状态
    private long lastClickTime = 0; // 单击时间
    private float startDistance; // 多点触摸两点距离
    private float endDistance; // 多点触摸两点距离

    private float topHeight; // 状态栏高度和标题栏高度
    private Bitmap primaryBitmap = null;

    private float contentW; // 屏幕内容区宽度
    private float contentH; // 屏幕内容区高度

    private float primaryW; // 原图宽度
    private float primaryH; // 原图高度

    private float scale; // 适合屏幕缩放倍数
    private Boolean isMoveX = true; // 是否允许在X轴拖动
    private Boolean isMoveY = true; // 是否允许在Y轴拖动
    ...
    private ICustomMethod mCustomMethod = null;
